...
public class MemoryMap<T> implements Iterable<MemoryRegion<T>> {
    ...
    public void add(MemoryRegion<T> newEntry) {
        ...
        if (regionOverlaps(newEntry, previousEntry)) {
            Main.log("MemoryMap", "WARNING: Overlapping entries: " + newEntry + "/" + previousEntry);
        }
        if (regionOverlaps(newEntry, nextEntry)) {
            Main.log("MemoryMap", "WARNING: Overlapping entries: " + nextEntry + "/" + newEntry);
        }
        if (!_treeSet.add(newEntry)) {
            MemoryRegion<T> existingEntry = _treeSet.floor(newEntry);
            assert existingEntry != null;
            Main.log("MemoryMap", "WARNING: Duplicate entry: " + existingEntry + "/" + newEntry);

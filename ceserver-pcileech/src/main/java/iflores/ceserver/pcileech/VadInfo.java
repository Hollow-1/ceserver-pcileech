...
public class VadInfo implements Serializable {
    ...
    public VadInfo(String name, long start, long end, int dword0, int dword1) {
        ...
        if(Settings.DEBUG) {
            Main.log("VadInfo","_name="+_name+",_start="+_start+
                ",_end="+_end+",_type="+_type+
                ",_protection="+_protection+",_fImage="+_fImage+
                ",_fFile="+_fFile+",_fPageFile="+_fPageFile+
                ",_fPrivateMemory="+_fPrivateMemory+",_fTeb="+_fTeb+
                ",_fStack="+_fStack+",_fSpare="+_fSpare+
                ",_HeapNum="+_HeapNum+",_fHeap="+_fHeap+
                ",_cwszDescription="+_cwszDescription+",_commitCharge="+_commitCharge+
                ",_memCommit="+_memCommit);
        }
    ...
    public int getWin32Protection() {
        if(_fImage != 0) {
            return Win32Constants.PAGE_EXECUTE_READWRITE;
        }
